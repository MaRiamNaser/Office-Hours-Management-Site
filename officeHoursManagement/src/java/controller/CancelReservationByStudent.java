...
public class CancelReservationByStudent extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Integer reservationID = 2;//Integer.parseInt(request.getParameter("reservationID"));
        try{
            boolean status = daoCancelResrvation.cancelReservation(reservationID);
            daoCancelResrvation.closeConnection();
            if (status == true){
                out.print("cancel operation done successfully");
            }
            else{
                out.print("cancel operation failed!");
            }
        }catch (Exception e) {
